 ...
 * @param defaultConfig
 * @param theConfig  可以使用_merge来控制对象类型的属性不合并(默认合并)
 */
export const mergeConfig = (defaultConfig, theConfig) => {
    ...
    Object.keys(defaultConfig).forEach(key => {
        ...
        } else if (dataTypeTest(newConfig[key]) === "object" && newConfig[key]._merge !== false) {
            //有该属性,并且这个属性是对象,没有设置不合并默认配置
            newConfig[key] = mergeConfig(defaultConfig[key], newConfig[key]);
        }
    });
    return newConfig;
};
